...
//一般用
	public List<PurchaseHistoryInfoDTO> getPurchaseHistoryList(String loginId) {
		...
		try {
			...
			while (resultSet.next()) {
				...
				purchaseHistoryInfoDto.setBirthPlace(resultSet.getString("birth_place"));
				purchaseHistoryInfoDto.setBirthDate(resultSet.getString("birth_date"));
	...
	//管理者用（全件）
		public List<PurchaseHistoryInfoDTO> getPurchaseHistoryList() {
			...
			String sql = "select"
					...
					+ " pi.birth_date," /* 生年月日 */
					+ " phi.price as subtotal," /* 値段 */
					+ " phi.destination_id as destination_id, "/*決済ID*/
					+ " phi.regist_date as regist_date," /* 登録日 */
					+ " phi.update_date as update_date," /* 更新日 */
					+ " di.family_name as family_name," /* 姓 */
					+ " di.first_name as first_name," /* 姓 */
	...
	public int regist(String loginId, int productId, int productCount, int destinationId, int price) {
		DBConnector dbConnector = new DBConnector();
		Connection connection = dbConnector.getConnection();
		String sql = "insert into purchase_history_info(user_id, product_id, product_count, price, destination_id, regist_date) values (?, ?, ?, ?, ?, now())";
		int count = 0;
		try {
			PreparedStatement preparedStatement = connection.prepareStatement(sql);
			preparedStatement.setString(1, loginId);
